        ...
        private List<Complex> LastStages(List<float> X_FirstStage)
        {
            ...
            for (int stage = 2; stage <= Math.Log(N) / Math.Log(2); stage++) // loop for each stage of fft 
            {
                ...
                for (int i = 0; i < DFTnum / 2; i++) // loop for each DFT in each stage 
                {
                    ...
                    {
                        Complex FH = ftHelper.ComplexMultiplication(new Complex(X[(Wdivide / 2) + m].Real, X[(Wdivide / 2) + m].Imaginary), 
                            new Complex((((float)Math.Cos((2 * Math.PI * n) / Wdivide))), (-1 * ((float)Math.Sin((2 * Math.PI * n) / Wdivide)))));
                        FH.Real = FH.Real + X[m].Real;
                        FH.Imaginary = FH.Imaginary + X[m].Imaginary;

                        Complex SH = ftHelper.ComplexMultiplication(new Complex(X[m + (Wdivide / 2)].Real, X[(Wdivide / 2) + m].Imaginary), 
                            new Complex(((float)Math.Cos((2 * Math.PI * (n + (Wdivide / 2))) / Wdivide)), (-1 * ((float)Math.Sin(2 * Math.PI * ((Wdivide / 2) + m) / Wdivide)))));
